...
def system_setup(cfg, args):
    ...
    if cfg.network == 'SpatialRectifier':
        ...
        model = SpatialRectifierResnetUnet(K=cfg.K, depth_estimation_cnn_ckpt=cfg.checkpoint_path, sr_cnn_ckpt=cfg.sr_checkpoint_path)
        print("=> loading Full_SR_DepthNet model '{}'".format(cfg.checkpoint_path))
    ...
    if cfg.loss == "SRLoss":
        ...
        loss_criteria = SR_DenseDepth_Full_Loss(w_net=1.0, w_pose=0.01, w_l1=0.1, w_ssim=1.0, w_grad=0.0, pose_mode='optimize')
...
def run_epoch(dataset, model, optimizer, loss_criteria, tb_logger, mode='train', epoch=0, K=None, args=None):
    ...
    #save every epoch lopps
    if epoch % 5 == 0:
        print('Saving the best checkpoint, epoch %d' % (epoch))
        path = os.path.join(cfg.model_dir, 'model-best_epoch_'+str(epoch)+'_' + cfg.train_dataset_split_id + '.pkl')
