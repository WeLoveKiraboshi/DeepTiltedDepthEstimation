...
import torchvision
import os
from models.GatedConv import GatedConv
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = GatedConv(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = GatedConv(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = GatedConv(planes, planes * 4, kernel_size=1, bias=False)
...
class UnpoolingAsConvolution(nn.Module):
    def __init__(self, inplanes, planes):
        ...
        self.conv_A = GatedConv(in_channels=inplanes, out_channels=planes, kernel_size=(3, 3), stride=1, padding=1)
        self.conv_B = GatedConv(in_channels=inplanes, out_channels=planes, kernel_size=(2, 3), stride=1, padding=0)
        self.conv_C = GatedConv(in_channels=inplanes, out_channels=planes, kernel_size=(3, 2), stride=1, padding=0)
        self.conv_D = GatedConv(in_channels=inplanes, out_channels=planes, kernel_size=(2, 2), stride=1, padding=0)
...
class UpProjection(nn.Module):
    def __init__(self, inplanes, planes):
        ...
        self.main_branch = nn.Sequential(
            ...
            GatedConv(planes, planes, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(planes)
        )
...
class ConConv(nn.Module):
    def __init__(self, inplanes_x1, inplanes_x2, planes):
        ...
        self.conv = GatedConv(inplanes_x1 + inplanes_x2, planes, kernel_size=1, bias=True)
...
class ResnetUnetHybridGatedConv(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], pretrained=False):
        self.inplanes = 64

        # resnet layers
        super(ResnetUnetHybridGatedConv, self).__init__()
        self.conv1 = GatedConv(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.conv2 = GatedConv(2048, 1024, 1, bias=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                GatedConv(self.inplanes, planes * block.expansion,
