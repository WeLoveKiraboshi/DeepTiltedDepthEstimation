...
from optimize import AlignmentDirectionOptimization
...
def vis2(data, is_show=False, is_save=True):
    ...
    WW = X.copy()
    for i in trange(len(X), desc='1st loop'):
        for j in trange(len(X[0]),desc='2nd loop', leave=False):
            x = X[i, j]
            y = Y[i, j]
            z = Z[i, j]
            WW[i, j] = near(np.array([x, y, z]), data, near_thr)
    ...
    if is_show:
        ...
        plt.show()
    if is_save:
        # save 3D gravity distribution info
        # XX, YY, ZZ, WW
        save_density_path = './aligned_data/e2_grav_density_rectified_2dofa.pkl'
        grav_density_dict = {'x': X, 'y': Y, 'z': Z, 'ww': WW, 'samples': data.shape[0], 'nearest_thr': near_thr}
        with open(save_density_path, "wb") as tf:
            pickle.dump(grav_density_dict, tf)
        tf.close()
...
def read_data_rectified_2dofa_pkl(datadir='/media/yukisaito/ssd2/ScanNetv2/scans', train_test_split='data/rectified_2dofa_scannet.pkl'):
    ...
    for iter, e2_comp in enumerate(tqdm(train_data['e2'])):
        subdir = e2_comp[:12]
...
def run_iteration(datadir='/media/yukisaito/ssd2/ScanNetv2/scans', train_test_split='data/rectified_2dofa_scannet.pkl', dataset=None):
    data_info = pickle.load(open(train_test_split, 'rb'))
    train_data = data_info['train']
    test_data = data_info['test']
    val_data = data_info['val']
    ...
    solver = AlignmentDirectionOptimization(dataset, )

    for iter, e2_comp in enumerate(tqdm(train_data['e2'])):
        subdir = e2_comp[:12]
        frame_idx = int(e2_comp[19:25])
        impath = os.path.join(datadir, subdir, 'color', str(frame_idx) + '.jpg')
        rgb_img = sio.imread(impath)
...
if __name__ == '__main__':
    ...
    # elif mode == 'test':
    #     dumped_data = pickle.load(open(density_file, 'rb'))
    #     print(dumped_data['x'])
    #     print(dumped_data['y'])
    #     print(dumped_data['z'])
    #     print(dumped_data['ww'])
    #     print(dumped_data['samples'])
