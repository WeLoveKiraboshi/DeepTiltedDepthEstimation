...
class Warping2DOFAlignment_np:
    def __init__(self, fx=577.87061*0.5, fy=577.87061*0.5, cx=319.87654*0.5, cy=239.87603*0.5):
        ...
        self.I3 = np.identity(3) #torch.tensor(np.identity(3), dtype=torch.float).to(self.device)
    ...
    def warp_all_with_gravity_center_aligned(self, x, I_g,  I_a): # Z -> depth
        x_i = x['image']
        x_m = x['mask']
        x_n = x['depth']
        C_g = x['gravity']
        ...
        w_x = {'image': [], 'mask': [], 'gravity': [], 'aligned_directions': [], 'depth':[], 'Cg_H_C_inv':[]}
        ...
        #print('x = {}, y = {}'.format(x_n.shape, y_n.shape))
        #print('Cg_R_C = {}'.format(Cg_R_C.shape))
        ...
        y_n = y_n.view(x_n.shape[0], x_n.shape[1], x_n.shape[2],  x_n.shape[3])
